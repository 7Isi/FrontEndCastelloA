  ...
  export class AuthService {
    ...
    //métodos
    registar(usuario:any): Observable<any> {
      return this.http.get<any[]>(
        `${this.apiUrl}?email=${usuario.email}`).pipe(
        switchMap( res => {
          if(res.length>0){
    ...
    //pegar chave de autenticação e usuário
    usuarioAtual(): any{
      return
      JSON.parse(localStorage.getItem(this.CHAVE_AUTH ) || "{}");
    }
